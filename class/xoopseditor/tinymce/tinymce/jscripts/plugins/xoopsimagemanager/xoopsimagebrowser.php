 ...
 * @version         $Id: xoopsimagebrowser.php 2812 2009-02-17 01:44:02Z dugris $
 */
...
echo '<title>{#xoopsimagemanager_dlg.dialog_title}</title>';
...
if ( $op == 'listimg' ) {
    echo '{#xoopsimagemanager_dlg.tab_listimages}';
} else {
    echo '{#xoopsimagemanager_dlg.tab_listcategories}';
}
echo '</a></span></li>';
if (!empty($catwritelist)) {
    echo '<li id="loadimage_tab"><span><a href="javascript:mcTabs.displayTab(\'loadimage_tab\',\'loadimage_panel\');" onmousedown="return false;">{#xoopsimagemanager_dlg.tab_loadimage}</a></span></li>';
}
if ( $admin ) {
    echo '<li id="createcategory_tab"><span><a href="javascript:mcTabs.displayTab(\'createcategory_tab\',\'createcategory_panel\');" onmousedown="return false;">{#xoopsimagemanager_dlg.tab_createcategory}</a></span></li>';
}
echo '</ul>';
echo '</div>';
...
echo '<div class="panel_wrapper">';
        ...
        if (!empty($catreadlist)) {
            ...
            for ($i = 0; $i < $catcount; $i++) {
                ...
                if ( in_array($imagecategories[$i]->getVar('imgcat_id'), array_keys($catreadlist)) ) {
                    ...
                    if ($admin && $imagecategories[$i]->getVar('imgcat_type') == 'C') {
    ...
    if ($op == 'listimg') {
        ...
        echo '<br /><br /><strong>{#xoopsimagemanager_dlg.select_image}</strong>';
    ...
    echo '<input type="button" id="cancel" name="cancel" value="{$lang_cancel}" onclick="tinyMCEPopup.close();" />';
    echo '</div>';
    echo '</div>';
