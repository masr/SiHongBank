...
case "list":
	...
	if (isset($_POST['page_index'])){
		...
		if (intval($_POST['page_index']) != $page_obj->getVar('page_index')){
	    	...
	    	if( !$page_obj->getVar('page_title') ){
	    		redirect_header('admin.page.php', 3, _AM_BUSSINESS_PAGE_ORDER_ERROR);
	    	}
    ...
    $xoopsTpl->display("db:bussiness_admin_page.html");
    break;
...
case "save":
    ...
    if ($page_handler->insert($page_obj)) {
        redirect_header('admin.page.php', 3, sprintf(_AM_BUSSINESS_SAVEDSUCCESS, _AM_BUSSINESS_PAGE_INSERT)); 
    }
...
case "delete":
    ...
    if (isset($_REQUEST['ok']) && $_REQUEST['ok'] == 1) {
        if($page_handler->delete($page_obj)) {
            ...
            redirect_header('admin.page.php', 3, sprintf(_AM_BUSSINESS_DELETESUCCESS, _AM_BUSSINESS_PAGE_INSERT));
        }else{
            echo $page_obj->getHtmlErrors();
        }
    }else{
        xoops_confirm(array('ok' => 1, 'id' => $page_obj->getVar('page_id'), 'op' => 'delete'), $_SERVER['REQUEST_URI'], sprintf(_AM_BUSSINESS_RUSUREDEL, $page_obj->getVar('page_menu_title')));
    }
    break;
}
