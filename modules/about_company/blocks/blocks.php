...
function about_company_block_menu_show(){
...
function about_company_block_page_show($options){    
	@include dirname(dirname(__FILE__)) . "/xoops_version.php";
	$block = array();
	$page_handler =& xoops_getmodulehandler('page', 'about_company');
...
function about_company_block_page_edit($options){
	@include dirname(dirname(__FILE__)) . "/xoops_version.php";
	$page_handler =& xoops_getmodulehandler('page', 'about_company');
	...
	$page_select = new XoopsFormRadio(_MI_ABOUT_COMPANY_BLOCKPAGE, 'options[0]', $options[0], '<br />');
	$page_select->addOptionArray($options_page);
	$form->addElement($page_select);
	$form->addElement(new XoopsFormText(_MI_ABOUT_COMPANY_TEXT_LENGTH, 'options[1]',5 , 5 ,$options[1]));
	$form->addElement(new XoopsFormText(_MI_ABOUT_COMPANY_VIEW_MORELINKTEXT, 'options[2]', 30, 50 , $options[2]));
	$form->addElement(new XoopsFormRadioYN(_MI_ABOUT_COMPANY_DOTITLEIMAGE, 'options[3]', $options[3]));
