 ...
 * @version         $Id: forms.php 2799 2009-02-14 07:11:55Z phppp $
 */
...
function profile_getRegisterForm(&$user, $profile, $step = null)
{
    ...
    $reg_form = new XoopsThemeForm($step['step_name'], "regform", $action, "post");
...
function profile_getUserForm(&$user, $profile = null, $action = false)
{
    ...
    if ($user->isNew() || $xoopsUser->isAdmin()) {
        $elements[0][] = array('element' => new XoopsFormText(_US_NICKNAME, 'uname', 25, $xoopsUser->isAdmin() ? 60 : $xoopsConfigUser['maxuname'], $user->getVar('uname', 'e')), 'required' => 1);
