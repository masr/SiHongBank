...
class ImageHandler {
	...
	function changeSizeByHeight($width, $height=-1) {
		if(!is_resource($this->imageResource)) {
			exit('不能改变图片的尺寸,可能是你没有设置图片来源.');
		}
		$s_width = $this->imageInfo[0];
		...
		$this->target = imagecreatetruecolor($height, $width);
		//如果比例不对，则取中间的部分
		if(abs($s_width/$s_height*$height-$width)>2){
			if(($s_height-$height/$width*$s_width)>0){//说明高度过高，需要在上下截取
				$height_diff=floor(($s_height-$s_width*$height/$width)/2);
		...
		if(@imagecopyresized($this->target, $this->imageResource, 0, 0, $height_diff ,$width_diff, $height,$width ,$s_height-2*$height_diff , $s_width-2*$width_diff)){
			return true;
		}else{
			return false;
		}
